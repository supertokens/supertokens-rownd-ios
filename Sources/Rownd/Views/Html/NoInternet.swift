    ...
    return """
        <html>
            <head>
                ...
                <style>
                    \(CSS(fontSize: fontSize, isDarkMode: isDarkMode, primaryColor: primaryColor))
                </style>
                <script>
                    function tryAgain() {
                        if (window?.webkit?.messageHandlers?.rowndIosSDK) {
                            ...
                            setTimeout(()=> {
                                window.webkit.messageHandlers.rowndIosSDK.postMessage('{"type":"\(MessageType.tryAgain.rawValue)"}');
                            }, 1000);
                        }
                    }
                </script>
...
func CSS(fontSize: CGFloat, isDarkMode: Bool, primaryColor: String) -> String {
    return """
        body {
